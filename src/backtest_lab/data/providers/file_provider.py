...
from ..base import DataProvider, MarketDataRequest, MarketData, DataFrequency, generate_filename, parse_filename
...
class FileDataProvider(DataProvider):
    ...
    def get_data(self, request: MarketDataRequest) -> MarketData:
        ...
        if not file_path:
            raise FileNotFoundError(f"No data file found for symbol {symbol} with frequency {frequency.value} in {self.data_directory}")
        ...
        if request.start_date or request.end_date:
            market_data = self._filter_by_date_range(market_data, request.start_date, request.end_date)
    ...
    def _add_provider_metadata(self, market_data: MarketData, file_path: Path, frequency: DataFrequency = None) -> None:
    ...
    def _find_file_for_symbol(self, symbol: str, frequency: DataFrequency = None) -> Optional[Path]:
    ...
    def _filter_by_date_range(self, market_data: MarketData, start_date: datetime = None, end_date: datetime = None) -> MarketData:
